...
def main():
    ...
    with tf.Session() as sess:
        ...
        # image = np.zeros((32, 32, 1), 'float32')
        image = images[0]
        intermediate_output = None
        for i in range(1, len(encoder.layers)):
            os.makedirs(f"../test/encoder_{i}", exist_ok=True)
            model = K.function([encoder.layers[0].input], [encoder.layers[i].output])
        ...
        for i in range(1, len(decoder.layers)):
            ...
            model = K.function([decoder.layers[0].input], [decoder.layers[i].output])
