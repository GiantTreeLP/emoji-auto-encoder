...
def encoder_128(vector_len: int) -> Model:
    ...
    x = Conv2D(16, (5, 5), activation=relu, kernel_initializer='VarianceScaling', padding='same', name="Convolution1")(
        input_img)
    x = MaxPooling2D((2, 2), padding='same', name="shrink_64x64")(x)  # 64x64
    x = Conv2D(8, (3, 3), activation=relu, kernel_initializer='VarianceScaling', padding='same', name="Convolution2")(x)
    x = MaxPooling2D((2, 2), padding='same', name="shrink_32x32")(x)  # 32x32
    x = Conv2D(8, (3, 3), activation=relu, kernel_initializer='VarianceScaling', padding='same', name="Convolution3")(x)
    x = MaxPooling2D((4, 4), padding='same', name="shrink_8x8")(x)
    x = Conv2D(4, (3, 3), activation=relu, kernel_initializer='VarianceScaling', padding='same')(x)
...
def decoder_128(vector_len: int) -> Model:
    ...
    x = Conv2D(8, (3, 3), activation=relu, kernel_initializer='VarianceScaling', padding='same')(x)
    x = UpSampling2D((2, 2), name="grow_16x16")(x)
    x = Conv2D(8, (3, 3), activation=relu, kernel_initializer='VarianceScaling', padding='same')(x)
    x = UpSampling2D((2, 2), name="grow_32x32")(x)
    x = Conv2D(8, (3, 3), activation=relu, kernel_initializer='VarianceScaling', padding='same')(x)
    x = UpSampling2D((2, 2), name="grow_64x64")(x)
    x = Conv2D(16, (3, 3), activation=relu, kernel_initializer='VarianceScaling', padding='same')(x)
    x = UpSampling2D((2, 2), name="grow_128x128")(x)
    decoded = Conv2D(1, (5, 5), activation=tanh, kernel_initializer='VarianceScaling', padding='same',
